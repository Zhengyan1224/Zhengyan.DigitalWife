...
internal sealed class GamePlayerGame : Zhengyan.DigitalWife.Mmd.Game.Game
{
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        if (TryDrawUnderwaterCameraToBackBuffer(gameTime, _camera, 0, 0, width, height, scissorEnabled: false, GraphicsDevice.ClearColor))
        {
            _renderedSceneThisFrame = true;
            return;
        }
    ...
    private bool TryDrawCameraViewports(GameTime gameTime)
    {
        ...
        foreach (SceneCameraSettings settings in viewportCameras)
        {
            ...
            DrawSceneSkybox(gameTime);
            _guiOverlay?.DrawBackgroundSprites(screenWidth, screenHeight, x, yTop, width, height);
    ...
    private void RenderShadowMap(
        ...
        Action? restoreRenderTarget = null)
    {
        if (_shadowMapRenderer is null || _localLightShadowRenderer is null)
        {
            return;
        }

        Action restore = restoreRenderTarget ?? GraphicsDevice.RestoreBackBuffer;
    ...
    private bool TryDrawUnderwaterCameraToBackBuffer(
        GameTime gameTime,
        OrbitCamera camera,
        int x,
        int y,
        ...
        Action bindOutputTarget)
    {
        ...
        DrawSceneSkybox(gameTime);
        _guiOverlay?.DrawBackgroundSprites(
            layoutWidth,
            layoutHeight,
            viewportX,
            viewportY,
            width,
            height);
        // Keep the surface in the capture so a below-water camera can see it.
        DrawSceneComponentsOnce(gameTime);
        ...
        out UnderwaterPostProcessSettings settings)
    {
        ...
        {
            WaterSurfaceSettings water = waterObject.Definition.Water;
    ...
    internal void ApplyRuntimeSettings()
    {
        ...
        bool useVulkanCompute = GraphicsDevice.Backend == GraphicsBackend.Vulkan
