...
internal static unsafe class DesktopSpritePlatform
{
    ...
    private static void SyncX11ClickThroughRegion(IWindow window, byte[] framebufferRgba, int width, int height)
    {
        ...
        // Keep the visual window rectangle stable. Updating ShapeBounding from
        // asynchronous readback can leave stale silhouettes behind while the
        // sprite moves; only the input region is needed for click-through.
