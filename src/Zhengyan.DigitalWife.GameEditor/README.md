# Zhengyan.DigitalWife.GameEditor
...
```text
src/Zhengyan.DigitalWife.GameEditor/bin/Debug/net10.0/GameEditorProjects/DemoGame
```
...
```powershell
dotnet run --project src/Zhengyan.DigitalWife.GamePlayer/Zhengyan.DigitalWife.GamePlayer.csproj -- <game-package.dwgame>
```

如果正式使用时不希望显示控制台窗口，见 [GamePlayer README](../Zhengyan.DigitalWife.GamePlayer/README.md#隐藏控制台--无终端启动)。

## 应用程序图标

Windows 构建会使用仓库内的 `assets/mmd/samples/GameData/Logo/favicon.ico` 作为 `GameEditor.exe` 的应用程序图标。该图标通过项目文件的 `ApplicationIcon` 配置嵌入到 Windows 可执行文件中。
...
cp assets/mmd/samples/GameData/Logo/favicon.png ~/.local/share/icons/hicolor/256x256/apps/zhengyan-digitalwife-gameeditor.png

mkdir -p ~/.local/share/applications
cat > ~/.local/share/applications/zhengyan-digitalwife-gameeditor.desktop <<'EOF'
[Desktop Entry]
Type=Application
Name=Zhengyan DigitalWife GameEditor
Exec=/opt/Zhengyan.DigitalWife.GameEditor/Zhengyan.DigitalWife.GameEditor
Icon=zhengyan-digitalwife-gameeditor
Terminal=false
...
请把 `Exec=` 改成你实际发布后的 GameEditor 可执行文件路径。如果使用 `dotnet Zhengyan.DigitalWife.GameEditor.dll` 方式启动，也可以把 `Exec=` 写成对应的 `dotnet ...dll` 命令。
...
`Project -> LLM / OpenAI-compatible` 中提供两组独立开关：
- `Enable skills tools`：让 GamePlayer 在调用 LLM 时自动注册 `skill_*` 工具，LLM 可以读取项目 `skills/` 目录、读取/写入项目文件、搜索文本，并在项目目录内执行命令。
- `Enable memory tools`：让 GamePlayer 自动注册 `memory_*` 工具，LLM 可以读写本地长期记忆，记忆文件保存在 save 目录下的 `memory/`。
...
启用后，C# 的 `Scene.Llm.ChatAsync/StreamChatAsync/StartChat/StartChatWithTools` 和 Python 的 `scene.llm.start_chat/start_chat_with_tools` 会自动带上当前已启用的内置工具。Python 的同步 `scene.llm.chat/stream_chat/stream_messages` 由 Python worker 直接请求 HTTP，目前不会执行 GamePlayer 的内置工具；运行时 UI 建议使用后台式 `start_chat`。
