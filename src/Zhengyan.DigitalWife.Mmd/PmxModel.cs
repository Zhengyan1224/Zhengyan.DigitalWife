...
using static Zhengyan.DigitalWife.Mmd.PmxMorph;
...
public unsafe class PmxModel : MMDModel
{
    ...
    public override bool Load(string path, string mmdDataDir)
    {
        ...
        foreach (PmxMaterial material in pmx.Materials)
        {
            ...
            if (material.SphereTextureIndex != -1)
            {
                ...
                {
                    // TODO: Handle SphereTexture sub-texture mode.
                }
            }
        ...
        // Morph
        foreach (Zhengyan.DigitalWife.Mmd.PmxMorph pmxMorph in pmx.Morphs)
        {
            PmxMorph morph = new()
            {
                Name = pmxMorph.Name,
            ...
            if (pmxMorph.MorphType == PmxMorphType.Position)
            {
                ...
                foreach (Zhengyan.DigitalWife.Mmd.PmxMorph.PositionMorph vtx in pmxMorph.PositionMorphs)
                {
            ...
            else if (pmxMorph.MorphType == PmxMorphType.UV)
            {
                ...
                foreach (Zhengyan.DigitalWife.Mmd.PmxMorph.UVMorph uv in pmxMorph.UVMorphs)
                {
                    UVMorph morphUV = new()
                    {
            ...
            else if (pmxMorph.MorphType == PmxMorphType.Bone)
            {
                ...
                foreach (Zhengyan.DigitalWife.Mmd.PmxMorph.BoneMorph pmxBoneMorph in pmxMorph.BoneMorphs)
                {
        ...
        IReadOnlyList<MMDRigidBody> rigidBodies = physicsManager.RigidBodies;
    ...
    public override void BeginAnimation()
    {
        ...
        if (_positionMorphDatas.Count != 0)
        {
            morphPositions.Fill(Vector3.Zero);
        }

        if (_uvMorphDatas.Count != 0)
        {
            morphUVs.Fill(Vector4.Zero);
        }
    }
